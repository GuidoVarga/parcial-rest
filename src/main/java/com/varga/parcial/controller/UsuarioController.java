...
public class UsuarioController {
    ...
    @PostMapping("")
    public void addUsuario(@RequestHeader(value="User-Agent") String userAgent, @RequestBody @NotNull @Valid Usuario usuario){
        usuario.setBrowser(userAgent);
        uRepository.save(usuario);
    }

    @PutMapping("/{id}")
    public void updateUsuario(@RequestHeader(value="User-Agent") String userAgent, @PathVariable("id") Integer id, @RequestBody @NotNull Usuario newUsuario){
        Usuario usuario = uRepository.findById(id).orElseThrow(() -> new HttpClientErrorException(HttpStatus.BAD_REQUEST));
